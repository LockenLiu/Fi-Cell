#########################################################################
# File Name: 3-get-sig-peak-gene-pair-and-snps-under-peak.r
# Author: LockenLiu
...
# cell='L3_5_IT' 
# dir='FiCell-Signac/precomputation/cisCor/L3_5_IT'
...
sclinker <- read.table(cts_file)
names(sclinker) <- c('Gene', 'score')
sclinker <- sclinker[sclinker$score > 0, ]
cisCor <- na.omit(merge(cisCor, sclinker, by.x = 'gene', by.y = 'Gene', all.x = TRUE))
