...
def find_snps_in_peak_bedtools(cisCor_bed, snp_bed):
    ...
    grouped_snps = intersected_df.groupby(['chr', 'start', 'end'])['SNP'].apply(lambda x: ', '.join(x)).reset_index()
    return grouped_snps
...
try:
    cisCor = pd.read_csv(os.path.join(cisCor_dir, f'{cell}.cisCor.tsv'), sep='\t')
    cisCor[['chr', 'start', 'end']] = cisCor['PeakRanges'].str.extract(r'chr(\d+|X|Y):(\d+)-(\d+)')
    ...
    output_file = os.path.join(cisCor_dir, f'{cell}.cisCor.snpped.tsv')
    cisCor.to_csv(output_file, sep='\t', index=False)
    print(f'Successfully saved: {output_file}')
except FileNotFoundError:
    print(f"Error: File {f'{cell}.cisCor.tsv'} not found in {cisCor_dir}.")
except pd.errors.EmptyDataError:
    print(f"Error: File {f'{cell}.cisCor.tsv'} is empty or corrupted.")
except Exception as e:
    print(f"Error processing file {f'{cell}.cisCor.tsv'}: {e}")
